...
In this regression analysis, we mainly focus on finding associations between the college attendance rate and several factors, including income, race, city type, and the number of children.
...
income: Annually Family Income in 10,000 dollars, from `r min(all$income)` to `r max(all$income)`.
...
```{r echo = FALSE, warning = FALSE, message = FALSE}
## Generate a graph showing the race composition respectively for each year from 2008 to 2018
all %>%
  group_by(year, race) %>%
...
```{r echo = FALSE, warning = FALSE, message = FALSE}
## Generate a bar graph showing geographic compositions for each year
all %>%
  group_by(year, city) %>%
...
```{r echo = FALSE, warning = FALSE, message = FALSE}
## Generate a graph showing the number of children distribution among subjects for each year
all %>%
  group_by(year, noc) %>%
...
$$ \hat{percent} = \beta_0 + \beta_1income + \beta_2race + \beta_3city + \beta_4noc + \varepsilon_i$$
...
$$ percent^* = \beta_0 + \beta_1income + \beta_2race + \beta_3city + \beta_4noc + \varepsilon_i$$
